...
use crate::{
    is_ident_continue, is_ident_start, Delimiter, Group, Ident, LexError, Literal, Punct, Spacing, Span, TokenStream,
    TokenTree,
};
    ...
    loop {
        ...
        let first = match input.bytes().next() {
            ...
            None => match stack.last() {
                ...
                Some(_frame) => return Err(LexError),
            },
        };
        ...
        if let Some(open_delimiter) = match first {
            ...
            input = input.advance(1);
            let g = Group::new(open_delimiter, TokenStream::from(trees));
            trees = outer;
            trees.push(TokenTree::Group(g));
        } else {
            let (rest, tt) = match leaf_token(input) {
                Ok((rest, tt)) => (rest, tt),
...
fn ident(input: Cursor) -> PResult<Ident> {
    if ["r\"", "r#\"", "r##", "b\"", "b\'", "br\"", "br#"]
        .iter()
...
fn ident_any(input: Cursor) -> PResult<Ident> {
    ...
    let ident = Ident::new_raw(sym, Span);
    Ok((rest, ident))
}
...
fn doc_comment(input: Cursor) -> PResult<Vec<TokenTree>> {
    ...
    let mut trees = vec![TokenTree::Punct(Punct::new('#', Spacing::Alone))];
    if inner {
